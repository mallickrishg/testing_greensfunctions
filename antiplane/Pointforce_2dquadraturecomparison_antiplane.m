...
N_gl = 61;
[xkvec,wkvec] = calc_gausslegendre_weights(N_gl);
[xk,yk] = meshgrid(xkvec,xkvec);
[wxk,wyk] = meshgrid(wkvec,wkvec);
wk = wxk.*wyk;

tic
parfor k=1:numel(xk)
    n_u = pointforce_disp(x_mat,y_mat,xk(k),yk(k))./area_source;
    n_s12 = mu.*pointforce_s12(x_mat,y_mat,xk(k),yk(k))./area_source;
    n_s13 = mu.*pointforce_s13(x_mat,y_mat,xk(k),yk(k))./area_source;
...
% numerical solution
h=0.01;% step size for tanh-sinh
n=fix(2/h);
...
set(findobj(gcf,'type','axes'),'Fontsize',15,'Linewidth',1.5,'TickDir','both')
%% compare residuals 
